  ...
  create: function create(meetingRef) {
    ...
    return new _javascriptStateMachine.default({
      ...
      }, // when leaving a meeting it must be from either the RINGING, JOINED, or ERROR states, and transitions it to the ENDED state
      {
        ...
        from: [_constants.MEETING_STATE_MACHINE.STATES.IDLE, _constants.MEETING_STATE_MACHINE.STATES.RINGING, _constants.MEETING_STATE_MACHINE.STATES.JOINED, _constants.MEETING_STATE_MACHINE.STATES.ANSWERED, _constants.MEETING_STATE_MACHINE.STATES.DECLINED, _constants.MEETING_STATE_MACHINE.STATES.ERROR, _constants.MEETING_STATE_MACHINE.STATES.SAFE],
      ...
      methods: {
        ...
        onBeforeError: function onBeforeError(transition, error) {
          _loggerProxy.default.logger.error("Meeting:state#onBeforeError --> state->onError#meeting.id: ".concat(this.meeting.id, " | Transition '").concat(transition.transition, "' : ").concat(transition.from, " -> ").concat(transition.to, " failed after last state transition, with error ").concat(error, ". Moving to a non-informative state. Last states: ").concat(this.history));
        },
